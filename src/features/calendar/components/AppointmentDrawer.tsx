...
export const AppointmentDrawer: React.FC<AppointmentDrawerProps> = ({
    ...
    useEffect(() => {
        ...
        if (!selectedAppointment) {
            ...
            const role = (user?.role || '').toUpperCase();
            if (role === 'ARTIST' || role === 'OWNER') {
                setIsEditing(false);
            } else {
                setIsEditing(true); // Managers go straight to edit (or should they also be read-only? User "stesse funzionalità previste per manager e artisti" suggests uniformity. Let's make everyone read-only first actually?)
                // User said: "Inoltre vorrei per Honor le stesse funzionalità previste per manager e artisti ... quando clicco ... si apre come sintesi ... solo dopo posso cliccare modifica"
                // This implies Managers ALREADY have this? Or he wants it FOR Honor (Owner) like Managers AND Artists?
                // The current code has `if (role === 'ARTIST') setIsEditing(false) else setIsEditing(true)`. So currently Managers edit immediately.
                // The user says "stesse funzionalità previste per manager e artisti". This implies he THINKS Managers already have it.
                // Safest bet: Make EVERYONE start in read-only (isEditing = false) when opening an existing appointment.
                setIsEditing(false);
            }
        }
